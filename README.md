# WorkShop---Fintech---BLOCKCHAIN

📄 Tu peux lire le **whitepaper original du Bitcoin** ici :  
👉 [Bitcoin: A Peer-to-Peer Electronic Cash System (Satoshi Nakamoto, 2008)](https://bitcoin.org/bitcoin.pdf)

---

## 💼 Hot Wallets

Exemple :
- [Binance](https://www.binance.com/fr) — plateforme d’échange permettant de gérer ses cryptos, faire du trading et du staking.  
- [MetaMask](https://metamask.io/) — extension de navigateur utilisée pour interagir avec les applications décentralisées (dApps).

---

## 🧊 Cold Wallets

Exemples :
- [Ledger](https://www.ledger.com/fr) — portefeuille matériel (hardware wallet) français reconnu pour sa sécurité et sa compatibilité avec +5000 cryptos.  
- [Tangem](https://tangem.com/fr/) — carte crypto physique, simple d’utilisation, sans batterie ni câble, compatible avec smartphone.

---

## 🧠 Bonnes pratiques

✅ Toujours sauvegarder ta **phrase de récupération (seed phrase)** sur papier, jamais en ligne.  
✅ Ne partage **jamais** ta clé privée.  
✅ Utilise un **cold wallet** pour le stockage long terme, et un **hot wallet** pour les transactions courantes.  
✅ Vérifie toujours les **liens officiels** avant de te connecter à un site crypto.

---

## 🛠 Outils Workshop

- [Remix – IDE Solidity en ligne](https://remix.ethereum.org/)  
- [Sepolia Faucet – Obtenir des ETH testnet](https://cloud.google.com/application/web3/faucet/ethereum/sepolia)

---

## 📚 Ressources utiles

- [Whitepaper Bitcoin (bitcoin.org)](https://bitcoin.org/bitcoin.pdf)  
- [Ledger – Site officiel](https://www.ledger.com/fr)  
- [Tangem – Site officiel](https://tangem.com/fr/)  
- [Binance – Plateforme d’échange](https://www.binance.com/fr)

---

🧾 **Auteur :** *Dimeo Zhang*  
📅 **Dernière mise à jour :** Octobre 2025
